package calc;

public class ComplexCalculator {

	private double currentNumber;
	
	// constructor
	public ComplexCalculator(double d) {
		currentNumber = d;
	}
	
	// adds d to current number
	public void add(double d) {
		currentNumber = currentNumber + d;
	}
	
	// subtract()
	// subtracts d from current number
	public void subtract(double d) {
		currentNumber = currentNumber - d;
	}
	
	// multiply()
	// multiplies current number by a given number
	public void multiply(double d) {
		currentNumber = currentNumber * d;
	}

	// divide()
	// divides current number by a given number
	public void divide(double d) {
		currentNumber = currentNumber / d;
	}
	// clear()
	// sets current number to 0
	public void clear() {
			currentNumber = 0;
	}
	
	// power()
	// power raises current number to the power of a given number
	public void power(double d) {
	//static methods do not need an instance of that class to be called
			Math.pow(currentNumber, d);
			
				double base = currentNumber;
			
				for(int i=0; i < d-1; i++) {
					currentNumber = base * currentNumber;
		double counter = d;}
		
	}
	public double getCurrentNumber() {
		return currentNumber;
	}
	
	//////////////////////////////////////////////////
	// EXTRA FUNCTIONS - not needed for full credit //
	
	// isEven()
	// TODO returns true is current number is even, false otherwise
	public boolean isEven() {
		
		if(currentNumber % 2 == 0)
			return true;
		// several options for this.  stay consistent
		return false;
	}
	
	// isPrime()
	// TODO returns true if current number is a prime number, false otherwise
	public boolean isPrime() {
		
		for(int i =2; i < currentNumber; i++){
			if(currentNumber % i == 0)
				return false;
			
		}
			return true;}}
		// several options for this.  stay consistent
	
	

